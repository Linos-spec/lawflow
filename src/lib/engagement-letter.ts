...
export async function generateEngagementLetterText(input: {
  ...
  try {
    const { text } = await generateText({
      model: aiModel,
      system: `You draft client engagement letters (retainer agreements) for the law firm "${input.firmName}". Produce a complete, professional letter in Markdown covering: the parties, scope of representation, the fee/retainer terms provided, billing and payment practices, client responsibilities, confidentiality, termination, and a signature block for client and attorney. Use standard, plain professional language. Use [bracketed placeholders] for anything not provided (effective date, addresses, hourly rates, jurisdiction). Begin with this note on its own line: "DRAFT — for attorney review. Verify scope and that the fee structure is permitted for this matter in your jurisdiction (e.g., contingency fees are prohibited in some matter types) before sending." Do not present it as executed or as legal advice to the client.`,
