...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await logAudit({ firmId: ctx.firmId, userId: ctx.userId, action: "invoice.create", entity: "Invoice", entityId: record.id, entityLabel: `${record.invoiceNumber} · ${record.client?.name ?? ""}`.trim(), details: `$${totalAmount.toLocaleString()}` });
