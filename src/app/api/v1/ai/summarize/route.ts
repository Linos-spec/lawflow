...
${caseData.billingRecords.map((b) => `  - ${b.invoiceNumber}: $${Number(b.totalAmount).toFixed(2)} (${b.paymentStatus})`).join("\n") || "  None"}
  ...
  const result = streamText({
    ...
    system: `You are a senior legal assistant AI for a law practice management system called Linoscore Legal. Generate a concise, professional case brief summary. Structure your response with these sections:
